...
RUN apt-get install -y  liblqr-1-0-dev   liblzma-dev libobjc-5-dev libobjc4 libopenexr-dev libpango1.0-dev  libpathplan4 libpcre3-dev libpcre32-3 libpcrecpp0v5 libperl-dev
RUN apt-get install -y libpixman-1-dev  libpotrace0 libptexenc1 libpthread-stubs0-dev   librsvg2-bin librsvg2-dev libsigsegv2 libsm-dev libsynctex1 libtexlua52
...
RUN apt-get install gnupg -yq && curl -sL https://deb.nodesource.com/setup_11.x | bash && apt-get install nodejs -yq
...
RUN sed -i '/short_open_tag/d' /etc/php/7.2/apache2/php.ini
RUN sed -i '/short_open_tag/d' /etc/php/7.2/cli/php.ini
ADD Docker-mihajlo.ini /etc/php/7.2/mods-available/mihajlo.ini
ADD Docker-xdebug.ini /etc/php/7.2/mods-available/xdebug.ini
ADD Docker-mongodb.ini /etc/php/7.2/mods-available/mongodb.ini
ADD Docker-vips.ini /etc/php/7.2/mods-available/vips.ini
ADD Docker-redis.ini /etc/php/7.2/mods-available/redis.ini
ADD Docker-memcached.ini /etc/php/7.2/mods-available/memcached.ini
...
RUN a2enmod ssl
RUN a2enmod expires
RUN echo "zend_extension=$(find /usr/lib/php/ -name xdebug.so)" >> /etc/php/7.2/mods-available/xdebug.ini
RUN echo "extension=$(find /usr/lib/php/ -name memcached.so)" >> /etc/php/7.2/mods-available/memcached.ini
RUN echo "extension=$(find /usr/lib/php/ -name redis.so)" >> /etc/php/7.2/mods-available/redis.ini
